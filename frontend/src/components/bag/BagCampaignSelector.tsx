...
const stripCombinedSuffix = (value: string) => value.replace(/\s*\(and\s+\d+\s+more\s+errors?\)\s*$/i, "")
const normalizeMessages = (value: unknown): string[] => {
  if (!value) return []
  const list = Array.isArray(value) ? value : [value]
  return list.map((item) => (typeof item === "string" ? stripCombinedSuffix(item.trim()) : "")).filter(Boolean)
}
const parseApiError = (payload: unknown): { general: string[]; fields: ErrorBag } => {
  if (!payload || typeof payload !== "object") return { general: [], fields: {} }
  const general = new Set<string>()
  const fields: ErrorBag = {}
  const data = payload as Record<string, unknown>
  if (typeof data.error === "string" && data.error.trim()) general.add(stripCombinedSuffix(data.error.trim()))
  if (typeof data.message === "string" && data.message.trim()) general.add(stripCombinedSuffix(data.message.trim()))
...
export function BagCampaignSelector({ activeCampaign, campaigns }: BagCampaignSelectorProps) {
  ...
  const handleSelect = (campaign: BagCampaign) => {
    ...
    selectMutation.mutate(campaign.id, {
      onSuccess: () => setFeedback({ type: "success", messages: [`${campaign.name} kampanyası uygulandı.`] }),
      onError: (error) => {
        const parsed = parseApiError((error as any)?.response?.data ?? (error as unknown as Record<string, unknown> | null))
        setFeedback({ type: "error", messages: parsed.general.length ? parsed.general : ["Kampanya uygulanamadı. Lütfen tekrar deneyiniz."] })
      },
    })
  }
  const handleUnselect = () => {
    ...
    unselectMutation.mutate(undefined, {
      onSuccess: () => setFeedback({ type: "success", messages: ["Kampanya kaldırıldı."] }),
      onError: (error) => {
        const parsed = parseApiError((error as any)?.response?.data ?? (error as unknown as Record<string, unknown> | null))
        setFeedback({ type: "error", messages: parsed.general.length ? parsed.general : ["Kampanya kaldırılamadı. Lütfen tekrar deneyiniz."] })
      },
    })
  }
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-3 overflow-hidden">
          <button
            className="bag-campaign-prev hidden sm:flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full border border-color bg-card shadow transition hover:border-[var(--accent)] hover:text-[var(--accent)] disabled:opacity-60 flex-shrink-0"
            type="button"
            disabled={isMutating}
          >
              ...
              campaigns.map((campaign) => {
                ...
                return (
                  <SwiperSlide key={campaign.id} className="!h-auto">
                    <div
                      className={`h-full rounded-xl border p-4 transition-all duration-200 flex flex-col justify-between ${
                        isActive
                          ? "border-emerald-500 bg-emerald-50 shadow-sm"
                          : "border-color bg-card hover:border-[var(--accent)]"
                      }`}
                    >
                      ...
                      <button
                        className="mt-3 rounded-md border border-color px-3 py-1.5 text-xs font-medium transition hover:border-[var(--accent)] hover:text-[var(--accent)] disabled:opacity-60"
          ...
          <button
            className="bag-campaign-next hidden sm:flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full border border-color bg-card shadow transition hover:border-[var(--accent)] hover:text-[var(--accent)] disabled:opacity-60 flex-shrink-0"
            type="button"
            disabled={isMutating}
          >

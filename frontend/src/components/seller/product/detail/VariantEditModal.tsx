...
export default function VariantEditModal({
  ...
  const isBusy = isSubmitting || createVariant.isPending || updateVariant.isPending
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 sm:p-4">
      <div className="w-full max-w-md sm:max-w-lg rounded-3xl bg-white p-4 sm:p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {!isEdit && (
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-3 sm:p-4 space-y-3">
              <h4 className="text-xs font-semibold text-gray-700">İlk Beden Bilgileri</h4>
              <div className="grid gap-3 sm:grid-cols-2">
                {[
                  ['create_size_option_id', 'Beden ID'],
                  ['create_size_price_cents', 'Fiyat (kuruş)'],
                  ['create_size_on_hand', 'Stok (on hand)'],
                  ['create_size_reserved', 'Reserved'],
                  ['create_size_min_stock', 'Minimum stok'],
                  ['create_size_warehouse_id', 'Depo ID'],
                ].map(([key, label]) => (
                  <div key={key}>
                    <label className="block text-[11px] font-medium text-gray-600">{label}</label>
                    <input
                      type="number"
                      className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm focus:border-gray-900 focus:outline-none"
                      {...register(key as keyof VariantEditFormValues, { valueAsNumber: true })}
                    />
                  </div>
                ))}
              </div>

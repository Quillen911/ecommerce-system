...
import { useUserAddressIndex } from '@/hooks/useUserAddressQuery'
import AddressFormModal from './AddressFormModal'
import { useUserAddressStore } from '@/hooks/useUserAddressQuery'
...
export default function AddressSelector({ onSelect, selectedAddressId }: AddressSelectorProps) {
    const { data: addresses, isLoading } = useUserAddressIndex()
    const [isModalOpen, setIsModalOpen] = useState(false)
    const storeMutation = useUserAddressStore()
    ...
    return (
        ...
        >
            ...
            <div className="flex justify-between items-center">
                <div>
                    <h3 className="text-2xl font-bold text-gray-900">Teslimat Adresi Seçin</h3>
                    <p className="text-gray-600 mt-1">Siparişinizin teslim edileceği adresi seçin</p>
                </div>
                <motion.button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {addresses?.map((address, index) => (
                    <motion.div
                        ...
                        className={`
                            relative border-2 rounded-2xl p-6 cursor-pointer transition-all duration-200 group
                    ...
                    >
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="text-gray-600 space-y-1">
                                ...
                                {address.address_line_2 && (
                                    <p className="ml-6 text-sm">{address.address_line_2}</p>
                                )}
                        ...
                        <div className={`
                            ...
                            ${selectedAddressId === address.id 
                                ...
                                : 'bg-gray-100 bg-opacity-0 group-hover:bg-opacity-30'
                            }
                        `} />
                    </motion.div>
                ))}
            ...
            {addresses?.length === 0 && (
                ...
                >
                    ...
                    <p className="text-gray-600 mb-6">Siparişinizi teslim almak için bir adres ekleyin</p>
                    <button 
                        onClick={() => setIsModalOpen(true)}
